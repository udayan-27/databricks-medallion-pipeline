...
- **Published dashboard** — Databricks SQL Dashboard **DE C1 E-Commerce Sales Dashboard**, created and rendered **manually in the Databricks UI**. Cursor did not generate the visual tiles. Sharing is **Anyone in my account can view** (account-scoped; not public internet access).
...
| Published dashboard | **DE C1 E-Commerce Sales Dashboard** rendered manually in the Databricks SQL UI (bar, histogram, pie/donut, two filters). Not generated by Cursor. |
...
4. Visual Dashboard creation/rendering in the Databricks SQL UI after Gold exists (`DASHBOARD_GUIDE.md`). SQL validation of those queries **is** automated; tile rendering is a UI operation and was completed manually. Cursor did not generate the visual dashboard.
...
Latest sequential full-suite result (this closeout): **Ran 223 tests in 840.713s OK**. The Databricks-workflow increment recorded 223 tests in 534.573s. The public-repo audit recorded 203 tests. Do not treat an older prompt-file timing as the current result. Local tests are not Databricks.
...
- Histogram binning and dashboard widgets are Databricks SQL UI operations. They were completed for the published dashboard; they are not produced by local parquet tests or by `run_pipeline.py`.
