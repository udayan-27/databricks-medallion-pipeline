...
Three evidence classes are distinct and must not be collapsed:

- **Local validation** — Spark / parquet tests in this repository (Python 3.11, JDK 17, PySpark 3.5.6).
- **Databricks validation** — `python src/databricks/run_pipeline.py` executed in the workspace. Bootstrap/source, Bronze, Silver, Gold, and dashboard SQL all **PASS**. Counts are recorded in [`FINAL_AUDIT.md`](FINAL_AUDIT.md).
- **Published dashboard** — Databricks SQL Dashboard **DE C1 E-Commerce Sales Dashboard**, created and rendered **manually in the Databricks UI**. Cursor did not generate the visual tiles. Sharing is **Anyone in my account can view** (account-scoped; not public internet access). The serialized Lakeview definition is version-controlled in [`dashboards/DE_C1_E-Commerce_Sales_Dashboard.lvdash.json`](dashboards/DE_C1_E-Commerce_Sales_Dashboard.lvdash.json).
...
| Serialized dashboard definition | [`dashboards/DE_C1_E-Commerce_Sales_Dashboard.lvdash.json`](dashboards/DE_C1_E-Commerce_Sales_Dashboard.lvdash.json), [`dashboards/README.md`](dashboards/README.md) |
...
| Bronze ingest | Implemented. Local parquet tests passed. Databricks / Delta Bronze **PASS** (`run_pipeline.py`). |
| Silver (all five modules + combiner) | Implemented. Local parquet tests passed. Databricks Silver **PASS**. |
| Gold SQL aggregations | Implemented. Local parquet tests passed. Databricks Gold **PASS**. |
| Dashboard SQL | Queries + guide implemented. Local Spark tests passed. Databricks dashboard SQL **PASS**. |
| Published dashboard | **DE C1 E-Commerce Sales Dashboard** rendered manually in the Databricks SQL UI (bar, histogram, pie/donut, two filters). Not generated by Cursor. Serialized definition exported to `dashboards/`. |
| Databricks workflow | Repository-owned bootstrap + pipeline + validation in `src/databricks/`. Uses existing Bronze/Silver/Gold modules. **Executed** in the workspace: `python src/databricks/run_pipeline.py`. |
...
Databricks execution from this repository: **completed** in the workspace with `python src/databricks/run_pipeline.py`. Local parquet results remain a separate evidence class. Visual tiles were created in the Databricks SQL UI after that run; they are not created by the Python command.
...
SQL: [`src/dashboard/dashboard_queries.sql`](src/dashboard/dashboard_queries.sql). Workspace click-path: [`src/dashboard/DASHBOARD_GUIDE.md`](src/dashboard/DASHBOARD_GUIDE.md). Serialized published definition: [`dashboards/DE_C1_E-Commerce_Sales_Dashboard.lvdash.json`](dashboards/DE_C1_E-Commerce_Sales_Dashboard.lvdash.json) (read-only export; see [`dashboards/README.md`](dashboards/README.md)).
...
This workflow **has been executed** in Databricks. Recorded result: bootstrap/source, Bronze, Silver, Gold, and dashboard SQL all **PASS**. Measured Gold facts: eligible revenue **46,083,475.86**; eligible order count **74,587**; segmentation population **10,000**. Full CHECK/EXPECTED/ACTUAL table: [`FINAL_AUDIT.md`](FINAL_AUDIT.md).
...
4. Visual Dashboard creation/rendering in the Databricks SQL UI after Gold exists (`DASHBOARD_GUIDE.md`). SQL validation of those queries **is** automated; tile rendering is a UI operation and was completed manually. Cursor did not generate the visual dashboard. The resulting serialized definition is in Git; publishing and account-level sharing remain workspace settings.
...
```
python -m unittest tests.test_bronze_contract tests.test_silver_contract tests.test_gold_contract tests.test_dashboard_contract tests.test_dashboard_artifact tests.test_databricks_workflow -v
```
...
| Dashboard artifact (Spark-free) | 9 |
| Dashboard Spark | 13 |
...
- Histogram binning and dashboard widgets are Databricks SQL UI operations. They were completed for the published dashboard; they are not produced by local parquet tests or by `run_pipeline.py`. The serialized definition is in `dashboards/`; warehouse binding and sharing ACLs are not fully stored there.
