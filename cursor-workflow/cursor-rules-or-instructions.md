...
- Current completed work: repository init + requirements/architecture/DQ design + Stage 2 sample data + Bronze ingest + **all five Silver quality modules and Silver table orchestration** + **Gold SQL aggregations and Gold table orchestration** + **Dashboard SQL queries and guide** + local Spark tests + Databricks compatibility **code review** + public-repository documentation audit + **repository-owned Databricks bootstrap/validation workflow** (`src/databricks/`) + **actual workspace execution of `run_pipeline.py`** + **published Databricks SQL dashboard** (manual UI; not generated by Cursor).
- Do not regenerate sample data unless asked.
- Do not modify pipeline logic, Gold SQL, dashboard SQL, or the published Databricks dashboard.
- Do not push until asked.
- Distinguish local parquet tests, Databricks `run_pipeline.py` PASS results, and the manually rendered dashboard. Do not apply `.venv` / winutils / parquet flags on the cluster. Do not document exploratory notebook cells as the official Databricks process.
